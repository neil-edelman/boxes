...
\usepackage{titling}
\pretitle{\fontfamily{\sfdefault}\fontsize{18bp}{18bp}\color{ilrblue}\selectfont}
\posttitle{\par\vspace{24bp}}
\preauthor{\normalfont\selectfont\MakeUppercase}
...
\renewenvironment{abstract}{%
    ...
    \else \small %
      \begin{center}%
        {\bfseries\color{ilrblue} \abstractname\vspace{\z@}}%
      \end{center}%
      \quotation
    \fi}
...
In practice, a set on a variable-length key, always terminated by a sentinel; this will allow a string and it's prefix in the same trie\cite{fredkin1960trie}. For proper ordering, we traverse with most-significant bit first and the sentinel should be less-than the encoding of all the characters; in practice, a null-terminated string. Figure~\ref{star-3:bits} shows a Modified~\mbox{UTF-8} example. It stores the keys in lexicographic order, but not by any collation algorithm.
...
A forest of trees. B-tree: always have the same number of trees.

\subsection{Structure in Memory}

Like a node in a B-trie, a tree structure is a contiguous chunk. A tree whose $\mathit{size}$, the number of nodes, is composed of internal nodes, $\mathit{branch}$, and external nodes, $\mathit{leaf}$; $\mathit{size} = \mathit{branch} + \mathit{leaf}$. As a non-empty complete binary tree, $\mathit{leaf} = \mathit{branch} + 1 \rightarrow \mathit{size} = 2\mathit{branch} + 1$. We arrange the branches pre-order in an array, thus, forward read in topologically sorted order. Number of branches remaining is initialized to the $b = \mathit{branch}$, we can make the tree semi-implicit by storing the $\mathit{left}$ branches and calculating $\mathit{right} = b - \mathit{left}$. As an example, refer to Figure~\ref{star-3:mem}.

The above wastes half of $\mathit{left}$'s dynamic range, in the best case, because branches that are $e$ from the end will have at most $\mathit{left} = e$: a tree with all left branches.
