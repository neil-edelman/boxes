...
Inline-chaining, as seen in Figure~\ref{types:inline}, is, in many ways, a hybrid between the two. The dark circles represent the closed heads, and the outline the open stack, with a highlight to indicate the stack position, compare to Figure~\ref{types:separate}. The expected number of dereferences is number of queries for plain-old-data. Because of the {\it next index} field, the space taken is one index {\it per} entry more then open-addressing. Being chained offers a higher load-factor, but it is not possible to exceed one: like open-addressing, the hash-table is contained in one block of memory.
...
\end{gnuplot}
\caption{A comparison of chained techniques.}%
\label{timing}%
\end{figure}%
