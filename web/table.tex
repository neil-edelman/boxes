...
Figure~\ref{timing} benchmarks straight insertion on a closed separately-chained set like Figure~\ref{types:separate}, an inline-chained set like Figure~\ref{types:inline}, and a {\it std::unordered\_set}. The data is a pointer to a randomly generated set of non-sense names, formed out of syllables, Poisson-distributed up to 15-letters in length; a \code{char[16]} with a null-terminator. Hashed by {\it djb2} to a 64-bit \code{size\_t}. The same data was used for each replica across different methods; collisions were ignored on all methods. The hash-tables were then destroyed for the next replica. The data was in a pool. Pre-allocation of the nodes of the separately-chained was in an array and was not counted towards the run-time.
...
This specific data for the average use-case shows the difference between separate-chaining and inline-chaining is not great enough to matter to performance.
