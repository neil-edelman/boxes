...
\abstract{Several design decisions for a minimal practical B-tree are discussed, covering bulk-loading, insertion, and deletion. Any allocations are fail-fast before modifying the tree so that it remains in a valid state. Concurrency not considered.}
...
\begin{figure}
    ...
    \caption{\code{remove} is the opposite of \code{add} in Figure~\ref{add}.}%
	\label{remove}%
\end{figure}%
