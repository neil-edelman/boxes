...
\title{Practical in-memory B-tree design}
\date{2022-08-05}
...
\abstract{Several design decisions for a minimal in-memory B-tree are discussed, covering bulk-loading, insertion, and deletion. Experimentally, a default order of 65 was reached based on insertions.}



\section{Design}

A tree is used as an ordered set or map. For memory locality, this is implemented B-tree\cite{bayer1972organization}. In an implementation for C, we would expect memory usage to be low, performance to be high, and simplicity over complexity. Practically, this means that B$^+$-trees and B$^*$-trees are less attractive, along with an added layer of order statistic tree. The nodes are linked one-way and iteration is very simple. This precludes multi-maps. The use-case has no concurrency and places importance on modification with operations being as lazy as is reasonably possible. Any allocations are fail-fast before modifying the tree so that it remains in a valid state. Concurrency not considered.

\subsection{Branching factor}

The branching factor, or order as \cite{knuth1998sorting}, is a fixed value between $[3,\text{UINT\_MAX}+1]$. The implementation has no buffering or middle-memory management. Thus, a high-order means greater memory allocation granularity, leading to asymptotically desirable caching. This conflicts with small values, which produce much more compact trees. The default order is 65, a value arrived at experimentally in Section~\ref{default}.

In general, it is left-leaning where convenient. This is
because keys on the right side are faster to move because of the array configuration of the nodes.
...
Bulk loading buffers, as described in \cite{graefe2011modern}, are too complex for our purposes. However, bulk loading is useful when the initial keys are in order. A key undergoing bulk-add is packed to the lowest height on the right side, ignoring the rules for splitting, and recruiting zero-key nodes below as needed. When the tree is full, the height increases.
...
\begin{figure}
	...
	absent [label="ABSENT", shape=none, style=none];
...
In \cite{bayer1972organization, knuth1998sorting, comer1979ubiquitous}, insertion is a $\mathcal{O}(\log \text{size})$ operation. In theory, the key is inserted in a leaf node. If this operation causes the leaf to be overfull, the leaf is split into two, with the median element promoted. This repeats until all the nodes are within the specified key maximum. If implemented directly, this requires an extra temporary element to be added to each node and double-linking to access the parent node.
...
In Figure~\ref{even}, the path from before \ref{even:work1} to \ref{even:insert} to insert 9 on Figure~\ref{add} is \code{descend}, \code{grow}, \code{split} four times (the maximum for this size), \code{fill}, and returns \code{ABSENT}. The new nodes are (branches) \code{Urbolgun}, \code{Beoub}, \code{Naagh}, and (leaf) \code{Olog}. This is the most complicated path, when the height increases.
...
Figure~\ref{remove} shows a schematic diagram of removal of one key. This has been done directly, working up the tree. Since nodes necessarily have the minimum amount of keys by \code{ascend}, we can write the a temporary pointer to the parent node of those that have the minimum number of keys (or less) in \code{lookup rm} and \code{branch} in the unused space for child nodes.

We require a leaf node for removal. In the event that the key is in a branch, we move the predecessor or successor, which is in a leaf, to the current spot and delete the leaf key. We must be careful to replace the provisional upwards-bubbling key with the predecessor or successor.

This is relying on being the only process which modifies the tree during deletion. A potentially more robust way would be from the top-down, like insertion. More work is needed to determine the optimal way of removing keys.
...
When the tree set has an extra value {\it per} key that is not associated with the tree operations, it becomes a map. To facilitate cache performance, these values are not interspersed with the keys. That is, only packed keys are ever accessed on lookup. The disadvantage of this approach is the mapped key and value are not in contiguous memory; on rotation of the tree, we must move two parts in tandem.

We expect some kind of map from an integral type to some kind of pointer to be the most common use-case. This covers polymorphic types. Figure~\ref{timing} and Figure~\ref{order} are using a map from \code{unsigned int} to pointer.

\subsection{Compared with $std::map$}
...
set grid
set xlabel "elements"
set ylabel "amortized time $per$ element, $t$ (ns)"
set yrange [0:2000]
set log x
set format x '\tiny %g'
set format y '\tiny %g'
set key font "modern,15"
set xrange [1:33554432]
set for [i=1:8] linetype i dashtype i
plot \
"timing-std.tsv" using 1:($2/$1*1000):($3/$1*1000) with errorlines title "std$\:\:$map" ls 1, \
"timing-o3.tsv" using 1:($2/$1*1000):($3/$1*1000) with errorlines title "order 3" ls 2, \
"timing-o4.tsv" using 1:($2/$1*1000):($3/$1*1000) with errorlines title "order 4" ls 3, \
"timing-o128.tsv" using 1:($2/$1*1000):($3/$1*1000) with errorlines title "order 128" ls 4, \
"timing-o257.tsv" using 1:($2/$1*1000):($3/$1*1000) with errorlines title "order 257" ls 5, \
"timing-o2049.tsv" using 1:($2/$1*1000):($3/$1*1000) with errorlines title "order 2049" ls 6
\end{gnuplot}
\caption{A comparison different orders and \code{std::map} in $C^{++}$ benchmark, with a log-$x$ scale. Lower $t$ numbers are better performing.}%
\label{timing}%
\end{figure}%

Figure~\ref{timing} shows the common case where one has an ordered map of \code{unsigned int} to pointer. It is of straight insertion of random elements, (not in any order.) The keys are drawn from \code{rand} in $[0, 2\,147\,483\,647]$. (This causes some probability of collision.)
...
set xtics 50
unset key
plot "order.tsv" using 1:($2/1000):($3/1000) with errorlines title "order" ls 1
\end{gnuplot}
\caption{Different orders benchmark for an addition of 100\,000 elements. Note that this is zoomed in on the region of interest. Especially, $t$ does not extend to zero; thus, the differences are more apparent.}%
\label{order}%
\end{figure}%

From Figure~\ref{timing}, we see that it resembles a level curve after the variance {\it per} element settles. This is what we would expect. When choosing a default fixed order, it is therefore appropriate to pick a fixed number of elements. Figure~\ref{order} is a cut at 100\,000 elements. This suggests that the default order be from 20 to 200.
...
A B-tree can be superior to a single node-based approach used in red-black or AVL trees, used by many languages to implement ordered sets or maps. It is not without cost, since this is now not a stable data-structure. A heuristic search in the average expected use-case revealed that an order 65 based node is a good choice.
