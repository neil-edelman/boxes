...
Deleting a non-leaf key involves merging \code{skips}. However, a key can be necessary to break up the entries in the trie into sub-maximum sized skips. In this implementation, one can not delete keys that have this property.
...
With smaller, dynamic tries, it is more important to not free resources which could be used in the future. Anything less than greedy merging on deletion will have hysteresis. We also should have a zero-key-state with resources, achieved with a flag on the tree size. In the implementation, this is branch size max, but it could be leaf size zero.
...
We are compressing by prefix.\cite{shannon1948mathematical}

It was shown in \cite{tong2016smoothed}
...
For an unordered set, a hash table is still hard to beat. If order is needed, then a B-tree. This is seen practice in, for example, \code{C$^{++}$23} where \code{std::unordered\_set} is commonly a hash table and \code{std::ordered\_set} is commonly a red-black tree. On top of that, if prefix matching is at all convenient, a Patricia trie is a really competitive solution.
