...
% Store the order instead of the branches; it's the same thing. Doesn't need to be in here.
...
set style line 1 lt 5 lw 2 lc rgb '#4fa6dc'
...
#set log y
plot \
"../timing/timing4.tsv" using 1:($3/1000):($4/1000) with errorlines title "10000 keys" ls 4, \
"../timing/timing5.tsv" using 1:($3/1000):($4/1000) with errorlines title "100000 keys" ls 3, \
"../timing/timing6.tsv" using 1:($3/1000):($4/1000) with errorlines title "1000000 keys" ls 1
\end{gnuplot}
\caption{The effects of order on run-time.}%
\label{timing}%
\end{figure}%

An octet is presumed to be a byte, if not, this will introduce some inefficiencies. An individual \code{left}, in multiples of octets, is either a maximum of 255, 65\,535, or more. 255 is a reasonable size for a contiguous tree. However, we reserve one flag, 254. This corresponds to the maximum branches of 255 and the maximum branching factor, or order, of 256.

Because of alignment, \code{skip} must also be one or three octets. We have made it one octet. One might also play around with the balance of \code{left} and \code{skip} ranges, but without a specific use case, this has been deemed acceptable. Bit fields are not as well-defined as bytes. The size of the tree fits nicely in the front, giving a maximum index size of 512 octets.
