...
After `exp(order)` the trie becomes saturated with `n-1` links?

only left and skip are descended in a binary search pattern, the leaf is the result. \code{skip} depends on the keys we are presented, but the maximum size is ideally \code{SIZE_MAX * CHAR_BITS - 1} to fit all strings that C uses. \code{left} is maximal at a graph of all left nodes, where it has `order - 2` value.
