...
int TR_(copy_if)(pT_(box) *restrict,
	const pTR_(box) *restrict, pTR_(predicate_fn));
static void TR_(keep_if)(pT_(box) *, pTR_(predicate_fn), pTR_(action_fn));
static void TR_(trim)(pT_(box) *, pTR_(predicate_fn));
#	endif
#endif
...
static int TR_(copy_if)(pT_(box) *restrict const dst,
	...
	if(!src) return 1;
	for(v = T_(data_at)(src, 0), end = v + T_(size)(src); v < end; v++) {
		/* Not falling/rising. */
...
static void TR_(keep_if)(pT_(box) *const box,
	...
	int keep0 = 1, keep1 = 0;
	assert(box && keep);
	for(v = T_(data_at)(box, 0), end = v + T_(size)(box); v < end;
		keep0 = keep1, v++) {
	...
	/* Adjust the size. */
	assert((size_t)(erase - T_(data_at)(box, 0)) <= T_(size)(box));
	T_(tell_size)(box, (size_t)(erase - T_(data_at)(box, 0)));
}
