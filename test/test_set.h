...
static void PS_(stats)(const struct S_(set) *const set, FILE *fp) {
	...
	size_t size = 0;
	if(set && set->entries) {
		PS_(index) idx, idx_end = 1 << set->log_capacity;
		for(idx = 0; idx < idx_end; idx++) {
			double delta, x;
			size_t displaced;
			if(!set->entries[idx].heir) continue;
	...
	if(set->entries) {
		...
		for(i = 0, i_end = 1 << set->log_capacity; i < i_end; i++) {
			const char *const bgc = i & 1 ? "" : " BGCOLOR=\"Gray90\"",
				*const top = set->top && PS_(index_value)(set->top) == i
				? " BORDER=\"1\"" : "";
...
static void PS_(histogram)(const struct S_(set) *const set,
	...
	if(set->entries) {
		...
		for(i = 0; i < i_end; i++) {
			size_t items;
			e = set->entries + i;
			if(!e->heir || set->top && PS_(index_value)(set->top) <= i
