...
struct ship;
struct ship_listlink;
struct ships;
typedef void (*ship_to_string_fn)(const struct ship *, char (*)[12]);
typedef void (*ship_action_fn)(struct ship *);
typedef void (*ships_action_fn)(struct ships *, struct ship *);
/** Virtual. */
struct ship_vt {
	const char type[16];
	const ship_to_string_fn to_string;
	const ships_action_fn delete;
};
/** Abstract. */
struct ship {
	const struct ship_vt *vt;
	char name[8];
};
/** @implements <ship>to_string */
static void ship_to_string(const struct ship *const ship,
	char (*const a)[12]) { ship->vt->to_string(ship, a); }
static void shiplink_to_string(const struct ship_listlink *, char (*)[12]);
#define LIST_NAME ship
#define LIST_EXPECT_TRAIT
#include "../src/list.h"
#define LIST_TO_STRING &shiplink_to_string
#include "../src/list.h"
#if 0
static struct ship *shiplink_upcast(struct ship_listlink *const link) {
	return (struct ship *)(void *)((char *)link
		- offsetof(struct ship_listlink, data));
}

static void shiplink_to_string(const struct ship_listlink *const link,
	char (*const a)[12]) { ship_to_string((const struct ship *)link, a); }
/* `destroyer` extends `ship`. */
struct destroyer {
	struct ship_listlink ship;
	unsigned no;
};
/** {container_of}. */
static struct destroyer *destroyer_upcast(struct ship *const ship) {
	return (struct Destroyer *)(void *)((char *)ship
		- offsetof(struct destroyer, ship) - offsetof(struct ship_listlink, data));
}
/** {const container_of}. */
...
static void battleships_tests(void) {
	unsigned seed = (unsigned)clock();
