...
static void pT_(valid_state)(const struct t_(deque) *const d) {
	struct pT_(block) *b;
	if(!d || !d->first) return;
...
static void pT_(test_basic)(void) {
	...
	T_(shrink)(&a);
	assert(DEQUE_MIN_CAPACITY > 1);
	assert(a.size == 1 && a.capacity == DEQUE_MIN_CAPACITY);
	item = T_(new)(&a); /* Add: 2. */
...
static void pT_(test_random)(void) {
	...
	for(i = 0; i < i_end; i++) {
		...
		if(a.size < 1000000 && !(i & (i - 1))) {
			char fn[128];
			printf("%s.\n", T_(to_string)(&a));
			sprintf(fn, "graph/array/" QUOTE(DEQUE_NAME) "-array-%lu.gv",
				(unsigned long)i);
			T_(graph_fn)(&a, fn);
		}
	}
...
static void T_(test)(void) {
	printf("<" QUOTE(DEQUE_NAME) ">array of type <" QUOTE(DEQUE_TYPE)
	...
	pT_(test_trim)();
	pT_(test_insert)();
	fprintf(stderr, "Done tests of <" QUOTE(DEQUE_NAME) ">array.\n\n");
}
