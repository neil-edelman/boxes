...
#ifndef LIST_L_NAME /* <-- !LIST_L_NAME */
...
#endif
#ifdef LIST_A_NAME
		"list A: <" QUOTE(LIST_A_NAME)
#ifdef LIST_A_COMPARATOR
		"> comparator <" QUOTE(LIST_A_COMPARATOR)
#endif
		">; "
#endif
#ifdef LIST_B_NAME
		"list B: <" QUOTE(LIST_B_NAME)
#ifdef LIST_B_COMPARATOR
		"> comparator <" QUOTE(LIST_B_COMPARATOR)
#endif
		">; "
#endif
#ifdef LIST_C_NAME
		"index C: <" QUOTE(LIST_C_NAME)
#ifdef LIST_C_COMPARATOR
		"> comparator <" QUOTE(LIST_C_COMPARATOR)
#endif
		">; "
#endif
#ifdef LIST_D_NAME
		"index D: <" QUOTE(LIST_D_NAME)
#ifdef LIST_D_COMPARATOR
		"> comparator <" QUOTE(LIST_D_COMPARATOR)
#endif
		">; "
#endif
		"testing:\n");
...
/** Counts the elements. */
static size_t PRIVATE_T_L_(count, elements)(struct T_(List) *const this) {
	size_t count = 0;
	T_(ListSetParam)(this, &count);
	T_L_(List, ShortCircuit)(this, &PRIVATE_T_L_(count, predicate));
	return count;
}
...
/** \see{PRIVATE_T_L_(exactly, elements)}
 @param param: struct <T>List<L>Verify
 @implements <T>Predicate */
...
static size_t PRIVATE_T_L_(exactly, elements)(struct T_(List) *const this,
	...
	return !T_L_(List, ShortCircuit)(this, &PRIVATE_T_L_(exactly, predicate));
}
...
static int PRIVATE_T_L_(in, order)(struct T_(List) *const this) {
	T *one_array[] = { 0 };
	T_(ListSetParam)(this, one_array);
	return !T_L_(List, ShortCircuit)(this, &PRIVATE_T_L_(order, predicate));
}
...
/** How many of them are not in order?
 @implements <T>Predicate */
static size_t PRIVATE_T_L_(count, unordered)(struct T_(List) *this) {
	struct PRIVATE_T_(Order) info = { 0, 0 };
	printf("Unordered(" QUOTE(LIST_NAME) "-" QUOTE(LIST_L_NAME) ": %s)\n",
		T_L_(List, ToString)(this));
	T_(ListSetParam)(this, &info);
	T_L_(List, ShortCircuit)(this, &PRIVATE_T_L_(unorder, predicate));
	return info.count;
}
#endif /* comp --> */

/** All elements are in the same array? */
static int PRIVATE_T_L_(in, array)(struct T_(List) *const this,
	const struct T_(ListNode) *const array, const size_t array_size) {
...
static void PRIVATE_T_L_(test, basic)(void) {
	...
	printf("Basic tests of " QUOTE(LIST_NAME) " linked-list " QUOTE(LIST_L_NAME)
		":\n");
	/* Clear */
	T_(ListClear)(0);
	T_(ListClear)(&a);
	...
	assert(!T_L_(List, ShortCircuit)(0, 0));
	assert(!T_L_(List, ShortCircuit)(0, &PRIVATE_T_L_(true, index)));
	assert(!T_L_(List, ShortCircuit)(&a, 0));
	assert(!T_L_(List, ShortCircuit)(&a, &PRIVATE_T_L_(true, index)));
	...
	assert(!T_L_(List, ShortCircuit)(0, 0));
	assert(!T_L_(List, ShortCircuit)(&a, 0));
	assert(!T_L_(List, ShortCircuit)(0, &PRIVATE_T_L_(every, second)));
	assert(T_L_(List, ShortCircuit)(&a, &PRIVATE_T_L_(every, second)) == buf + 1);
	/* GetNext, GetPrevious, GetFirst, GetLast */
	printf("Removing 3 elements from a.\n");
	assert(PRIVATE_T_L_(exactly, elements)(&a, buf, buf_size));
	assert(!T_L_(List, GetFirst)(0));
	assert(!T_L_(List, GetLast)(0));
	assert(!T_L_(ListNode, GetPrevious)(0));
	assert(!T_L_(ListNode, GetNext)(0));
	item_a = T_L_(List, GetFirst)(&a);
	assert(!T_L_(ListNode, GetPrevious)(item_a));
	item_b = T_L_(ListNode, GetNext)(item_a);
	item_z = T_L_(List, GetLast)(&a);
	assert(!T_L_(ListNode, GetNext)(item_z));
	item_y = T_L_(ListNode, GetPrevious)(item_z);
	...
	T_L_(List, ForEach)(0, &PRIVATE_T_L_(count, another));
	T_L_(List, ForEach)(&a, 0);
	T_L_(List, ForEach)(&a, &PRIVATE_T_L_(count, another));
	assert(PRIVATE_T_L_(count, var) == new_buf_size);
	assert(PRIVATE_T_L_(count, elements)(&a) == new_buf_size);
#ifdef LIST_SOME_COMPARATOR /* <-- comp */
	/* <L>Sort */
	printf("Sorting a only by " QUOTE(LIST_L_NAME) ".\n");
	T_L_(List, Sort)(0);
	T_L_(List, Sort)(&a);
	...
	T_(ListSort)(0);
	T_(ListSort)(&a);
	printf("Sorting, a = %s.\n", T_L_(List, ToString)(&a));
	assert(PRIVATE_T_(in_order)(&a));
#endif /* comp --> */
	/* ToString (unchecked) */
	printf("ToString: null = %s; a = %s.\n",
		T_L_(List, ToString)(0), T_L_(List, ToString)(&a));
	printf("Clear.\n");
	T_(ListClear)(&a);
...
static void PRIVATE_T_L_(test, memory)(void) {
	...
	for(i = 0; i < buf_size; i++) T_(ListAdd)(&a, buf[0] + i);
	for(i = 0; i < buf_size; i++) T_(ListAdd)(&a, buf[1] + i);
#ifdef LIST_SOME_COMPARATOR /* <-- comp */
	T_(ListSort)(&a);
	...
	printf("Spliting odd/even a to b = %s by " QUOTE(LIST_L_NAME) ";\n",
		T_L_(List, ToString)(&b));
	T_(ListSetParam)(&a, &is_parity);
	is_parity = 0;
	T_L_(List, TakeIf)(&b, &a, &PRIVATE_T_L_(every, second));
	printf("a = %s, b = %s.\n",
		T_L_(List, ToString)(&a), T_L_(List, ToString)(&b));
#ifdef LIST_SOME_COMPARATOR /* <-- comp */
...
#ifdef LIST_L_COMPARATOR /* <-- comp */
	/* Compare */
	assert(!T_L_(List, Compare)(0, 0));
	assert(T_L_(List, Compare)(&a, 0) > 0);
	assert(T_L_(List, Compare)(0, &b) < 0);
	assert(!T_L_(List, Compare)(&a, &b));
#endif /* comp --> */
	/* Test done. */
	T_L_(List, TakeIf)(&b, &a, 0);
	...
	/* Test done; move. */
	for(i = 0; i < buf_size; i++)
		T_(ListAdd)(&a, buf[0] + i), T_(ListAdd)(&b, buf[1] + i);
...
#ifdef LIST_L_COMPARATOR /* <-- compare */
...
static void PRIVATE_T_L_(test, boolean)(void) {
	...
	printf("Three distinct " QUOTE(LIST_L_NAME) "-elements: "
	...
	T_(ListAdd)(&a, &x[0].a), T_(ListAdd)(&a, &x[1].a);
	T_(ListAdd)(&b, &x[0].b), T_(ListAdd)(&b, &x[2].b);
	printf("Two " QUOTE(LIST_L_NAME) "-sequences, %s and %s.\n",
		   T_L_(List, ToString)(&a), T_L_(List, ToString)(&b));
	/* verify comparing things first */
	printf("Comparing-" QUOTE(LIST_L_NAME) " a = %s and c = %s.\n",
		T_L_(List, ToString)(&a), T_L_(List, ToString)(&c));
	assert(T_L_(List, Compare)(&a, &c) == 1);
	assert(T_L_(List, Compare)(&c, &a) == -1);
	assert(T_L_(List, Compare)(&a, &a) == 0);
	/* - */
	printf("(a = %s) - (b = %s) =\n", T_L_(List, ToString)(&a),
		T_L_(List, ToString)(&b));
	T_L_(List, TakeSubtraction)(&c, &a, &b);
	printf("%s; (a = %s, b = %s.)\n", T_L_(List, ToString)(&c),
		T_L_(List, ToString)(&a), T_L_(List, ToString)(&b));
	T_(ListAdd)(&ia, &x[0].ia);
	T_(ListAdd)(&ib, &x[0].ib), T_(ListAdd)(&ib, &x[2].ib);
	T_(ListAdd)(&ic, &x[1].ia);
	assert(!T_L_(List, Compare)(&a, &ia));
	assert(!T_L_(List, Compare)(&b, &ib));
	assert(!T_L_(List, Compare)(&c, &ic));
	T_(ListClear)(&a), T_(ListClear)(&b), T_(ListClear)(&c);
	T_(ListClear)(&ia), T_(ListClear)(&ib), T_(ListClear)(&ic);
	/* u */
	T_(ListAdd)(&a, &x[0].a), T_(ListAdd)(&a, &x[1].a);
	...
	printf("(a = %s) u (b = %s) =\n", T_L_(List, ToString)(&a),
		T_L_(List, ToString)(&b));
	T_L_(List, TakeUnion)(&c, &a, &b);
	printf("%s; (a = %s, b = %s.)\n", T_L_(List, ToString)(&c),
		T_L_(List, ToString)(&a), T_L_(List, ToString)(&b));
	T_(ListAdd)(&ib, &x[0].ib);
	T_(ListAdd)(&ic, &x[0].ia);
	if(PRIVATE_T_L_(data, cmp)(&x[1].ia.data, &x[2].ib.data) < 0) {
		T_(ListAdd)(&ic, &x[1].ia), T_(ListAdd)(&ic, &x[2].ib);
	} else {
		T_(ListAdd)(&ic, &x[2].ib), T_(ListAdd)(&ic, &x[1].ia);
	}
	assert(!T_L_(List, Compare)(&a, &ia));
	assert(!T_L_(List, Compare)(&b, &ib));
	assert(!T_L_(List, Compare)(&c, &ic));
	...
	printf("(a = %s) n (b = %s) =\n", T_L_(List, ToString)(&a),
		T_L_(List, ToString)(&b));
	T_L_(List, TakeIntersection)(&c, &a, &b);
	printf("%s; (a = %s, b = %s.)\n", T_L_(List, ToString)(&c),
		T_L_(List, ToString)(&a), T_L_(List, ToString)(&b));
	T_(ListAdd)(&ia, &x[1].ia);
	T_(ListAdd)(&ib, &x[0].ib), T_(ListAdd)(&ib, &x[2].ib);
	T_(ListAdd)(&ic, &x[0].ia);
	assert(!T_L_(List, Compare)(&a, &ia));
	assert(!T_L_(List, Compare)(&b, &ib));
...
static void PRIVATE_T_L_(test, order)(void) {
	...
	size_t i;
	T_(ListClear)(&a), T_(ListClear)(&b);
	assert(T_L_(List, Compare)(0, 0) == 0);
	assert(T_L_(List, Compare)(&a, 0) > 0);
	assert(T_L_(List, Compare)(0, &b) < 0);
	assert(T_L_(List, Compare)(&a, &b) == 0);
	for(i = 0; i < buf_size; i++) node = buf + i, PRIVATE_T_(filler)(&node->data);
	for(i = 0; i < buf_size >> 1; i++) T_(ListAdd)(&a, node--);
	assert(T_L_(List, Compare)(&a, &b) > 0);
	for( ; i < buf_size; i++) T_(ListAdd)(&b, node--);
	...
	T_(ListTake)(&a, &b);
	printf("Testing " QUOTE(LIST_NAME) "-" QUOTE(LIST_L_NAME) " a, b for order "
	...
	printf("Testing " QUOTE(LIST_NAME) "-" QUOTE(LIST_L_NAME) " a, b for order "
		...
		T_L_(List, ToString)(&a), T_L_(List, ToString)(&b));
#ifdef LIST_A_COMPARATOR
	printf("By " QUOTE(LIST_A_NAME) ": a = %s, b = %s.\n",
		T_LA_(List, ToString)(&a), T_LA_(List, ToString)(&b));
#endif
#ifdef LIST_B_COMPARATOR
	printf("By " QUOTE(LIST_B_NAME) ": a = %s, b = %s.\n",
		T_LB_(List, ToString)(&a), T_LB_(List, ToString)(&b));
#endif
#ifdef LIST_C_COMPARATOR
	printf("By " QUOTE(LIST_C_NAME) ": a = %s, b = %s.\n",
		T_LC_(List, ToString)(&a), T_LC_(List, ToString)(&b));
#endif
#ifdef LIST_D_COMPARATOR
	printf("By " QUOTE(LIST_D_NAME) ": a = %s, b = %s.\n",
		T_LD_(List, ToString)(&a), T_LD_(List, ToString)(&b));
#endif
	T_(ListMerge)(&a, &b);
	printf("Testing " QUOTE(LIST_NAME) "-" QUOTE(LIST_L_NAME) " a, b for order "
...
static void PRIVATE_T_L_(test, meta)(void) {
	...
	while(lists_left) {
		...
		T_(ListClear)(list);
		while(take) T_(ListAdd)(list, node++), take--;
		lists_left--;
		printf("%lu. %s\n", (unsigned long)(lists_size - lists_left),
			T_L_(List, ToString)(list));
	}
	printf("Sorting with qsort by " QUOTE(LIST_L_NAME) ".\n");
	qsort(lists, lists_size, sizeof *lists,
		(int (*)(const void *, const void *))&T_L_(List, Compare));
	for(i = 0; i < lists_size; i++) {
		printf("%lu. %s\n", (unsigned long)(i + 1),
			T_L_(List, ToString)(lists + i));
		if(i) { /* like {strcmp} comparing the first letter -- good enough */
			const struct T_(ListNode) *const less = lists[i - 1].L_(first),
				*const more = lists[i].L_(first);
			if(!less) continue;
